...
- フロントエンド全般: 初心者
- Rust: 初心者
- バックエンドエンジニアとしての基礎: 十分にある (サーバー、DB、APIの概念は理解済み)

**AIへの行動指針:**
1. コードを勝手に生成・上書きしない。実装のヒントとステップを示し、本人に書かせる
2. TS/Hono/フロントエンドの概念は「なぜそうなっているか」を丁寧に説明する
3. バックエンドの知識をアナロジーとして使ってよい (例: 「Express のミドルウェアと同じ考え方で...」)
4. 専門用語を使う場合は初出時に必ず一言説明を添える
5. レビュー時は「型安全」「Cloudflare Workersの制約」「Rustの慣習」の観点を含める
