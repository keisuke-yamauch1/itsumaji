...
async function syncRss(env: CloudflareBindings) {
    ...
    const episodeStatements = rssEpisodes.map((rssEpisode) =>
       ...
       VALUES (?, ?, ?, ?, ?, ?)`
        ).bind(rssEpisode.episode.guid, rssEpisode.episode.title, rssEpisode.episode.description, rssEpisode.episode.published_at, rssEpisode.episode.duration, rssEpisode.episode.thumbnail_url)
    )
